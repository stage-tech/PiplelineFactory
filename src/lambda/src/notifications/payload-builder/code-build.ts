...
export class CodeBuildPayloadBuilder implements INotificationsPayloadBuilder {
  constructor(private gitHubClient: GithubClient, private event: PipelineEventDetail) {}

  async buildNotificationPayload(): Promise<NotificationPayload> {
    const environmentVariables = this.event.sourceEvent.detail['additional-information'].environment[
