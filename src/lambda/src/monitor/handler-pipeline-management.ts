      ...
      await Promise.all(
        event.Records.map(async (sqsMessage) => {
          try {
            ...
            console.debug(
              `Retrieved Information using token ${JSON.stringify(
                organizationInfo.githubToken.substring(0, 5),
                null,
                4,
              )}`,
            );
            ...
            const existingBranches = await cloudFormationManager.getBranchesWithStacks(
              repository.owner,
              repository.name,
            );
