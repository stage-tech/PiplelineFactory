...
export default class DefaultBuckets extends Construct {
  ...
  constructor(scope: Construct, id: string, props: DefaultBucketsProps) {
    ...
    this.transientArtifactsBucket = new s3.Bucket(this, 'transientBucket', {
      bucketName: `${stack.stackName.toLowerCase()}-${stack.account}-${stack.region}-transient`,
    ...
    if (props.existingBucketName) {
      this.artifactsBucket = s3.Bucket.fromBucketName(this, 'existingArtifactsBucket', props.existingBucketName);
    } else {
      this.artifactsBucket = new s3.Bucket(this, 'artifactsBucket', {
        bucketName: `${stack.stackName.toLowerCase()}-${stack.account}-${stack.region}-artifacts`,
