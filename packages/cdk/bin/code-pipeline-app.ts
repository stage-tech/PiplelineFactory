...
const alternativeName = process.env.ALTERNATIVE_NAME;
...
const projectName = Utility.sanitizeStackName(`${githubRepositoryName}-${alternativeName ?? branchName}`).toLowerCase();
...
new CodePipelineStack(app, `PLF-${projectName}`, {
  ...
  envName: alternativeName ?? branchName,
